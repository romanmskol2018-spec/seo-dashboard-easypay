...
async function fetchReferrals(
  ...
  const params = new URLSearchParams({
    ids: counter,
    metrics: ["ym:s:visits,ym:s:users", ...goalMetrics].join(","),
    dimensions: "ym:s:referer,ym:s:date",
  ...
  if (!res.ok) {
    ...
    if (text.includes("too complicated") && date1 < date2) {
      ...
      return [...(await fetchReferrals(counter, date1, fmt(mid), token, goalIds)),
              ...(await fetchReferrals(counter, fmt(next), date2, token, goalIds))];
    }
    throw new Error(`HTTP ${res.status}: ${text.slice(0, 160)}`);
  }
  const json = await res.json();
  return (json.data || []).map((row: { dimensions: { name: string }[]; metrics: number[] }) => {
    let leads = 0;
    for (let i = 2; i < row.metrics.length; i++) leads += row.metrics[i] || 0;
    return {
      referer: row.dimensions[0].name || "",
